# Regu- 📰 **智能新聞擷取** - 從 Google News RSS 自動獲取熱門新聞
- 🤖 **AI 創作腳本** - 使用 Google Gemini AI 生成幽默四格漫畫
- 🔊 **語音播報** - 將腳本轉換為高質量中文語音
- 🌐 **智能界面** - 提供友好的命令行和網頁界面，自動篩選有效內容
- 📱 **響應式設計** - 支援桌面和移動設備
- ⚙️ **一鍵啟動** - 多種啟動方式，操作簡單
- 📁 **自動管理** - 智能文件組織和清理功能，只顯示有內容的批次ix
...
- 📱 **響應式設計** - 支援桌面和移動設備
- ⚙️ **一鍵啟動** - 多種啟動方式，操作簡單
- 📁 **自動管理** - 智能文件組織和清理功能，只顯示有內容的批次
- 🔄 **智能同步** - 一鍵將本地內容同步到 GitHub Pages (**v1.2.0 新功能**)
- 🤖 **自動化部署** - GitHub Actions 每3小時自動生成新內容
...
當您在本地端生成新的四格漫畫後，有多種方法可以查看和同步結果：

### 🎯 方法 1：智能同步工具（推薦）

**🚀 使用 `sync_to_github.bat` 自動同步**
```bash
# Windows 系統直接執行
./sync_to_github.bat
```
...
這個工具會：
1. 📁 自動掃描 `docs/outputs/` 目錄下的所有批次
2. 📄 智能更新 `file-list.json`，確保網頁能載入所有內容  
3. 🔄 詢問是否要同步到 GitHub Pages
4. 🚀 自動執行 git add、commit、push
5. 🌐 讓您的手動產生內容出現在線上網站

**適用時機：**
- ✅ 手動產生內容後想要同步到 GitHub Pages
- ✅ 確保線上網站顯示最新的本地內容
- ✅ 自動化 git 提交和推送流程
- ✅ 智能掃描並更新檔案索引

### 方法 2：靜態網站 + Live Server（推薦）
...
| 特點 | 智能同步工具 (方法 1) | 靜態網站 (方法 2) | 動態網站 (方法 3) |
|------|---------------------|------------------|------------------|
| **設定難度** | 非常簡單 | 簡單 | 簡單 |
| **GitHub Pages 同步** | ✅ 自動同步 | ❌ 需手動 git 操作 | ❌ 僅本地 |
| **檔案更新** | 🔥 智能掃描 | 需執行 `update_content.bat` | 自動掃描檔案 |
| **線上可見性** | ✅ 立即在 GitHub Pages 可見 | ❌ 需手動推送 | ❌ 僅本地可見 |
| **依賴** | Git + Python | VS Code Live Server 或瀏覽器 | Flask |
| **效能** | 中等 | 快速 | 稍慢 |
| **適用場景** | 📱 想要分享到線上 | 🔧 本地快速查看 | 🛠️ 本地開發測試 |
| **推薦度** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ |
...
### v1.2.0 智能同步版 (2025-07-01) 🆕

#### ✨ 新功能
- **🔄 智能同步工具** - 一鍵將本地內容同步到 GitHub Pages
- **📊 優化批次選擇器** - 網頁界面只顯示有內容的批次，提升用戶體驗
- **🤖 自動化增強** - GitHub Actions 每3小時自動生成10則新聞漫畫
- **📁 智能文件管理** - 自動掃描並更新檔案索引，確保網頁顯示最新內容

#### 🔧 改進
- **同步流程優化** - `update_file_list.py` 提供完整的掃描與同步功能
- **批次檔增強** - `sync_to_github.bat` 一鍵啟動同步流程
- **網頁界面優化** - 智能篩選有效批次，避免空白選項
- **自動化頻率調整** - 從每小時改為每3小時，平衡內容更新與資源使用

#### 🌐 GitHub Pages 整合
- **即時同步** - 本地生成的內容可立即同步到線上
- **智能掃描** - 自動發現 `docs/outputs/` 下的所有有效批次
- **用戶友好** - 提供清楚的操作指引和進度提示
- **穩定運行** - 解決自動 commit/push 問題，確保內容持續更新

#### 📈 當前狀態
- **總批次數**: 18 個有內容的批次
- **自動化**: GitHub Actions 每3小時執行
- **同步狀態**: 本地與線上完全同步
- **訪問網站**: https://sheng-luen-chung.github.io/regular-comix/
